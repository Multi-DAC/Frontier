...
| **6a** | ⛔ **NEW LOCI — the two Clayton chose, and they are different jobs** | ⛔ **PRE-REGISTERED, NOT RUN** | `PREREG-TERRESTRIAL.md`. **L1 NEXRAD-`CFP`**: non-stationary residual in the deletion counter, T1–T5, with a pre-committed **ceiling** — a perturbation measure cannot yield an object claim, ever. **L2 camera-trap blanks**: the only locus where the *human* is inside the instrument, C1–C4, human-blank ∩ detector-positive. Both expected outcomes written **pointing at the boring answer** before looking |
