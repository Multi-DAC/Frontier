...
2a. ⛔ **NEW — the three things pass 7 owes**, each needing its own pre-registration *before* the code, same as this one: **(i)** the B2 resolution — a third de-doubling that keeps unpaired negatives, so §8's amendment stops being provisional; **(ii)** the §9a re-test — `PASS7_DIAG.json` Q2 measured `sep` varying **in-process** (1014·1014·1015·1014·1016·1012) and **not at all across fresh processes** (1014×4), which is a sharper mechanism than §9a states and would make the re-run-and-diff gauge *unable to fail*, not merely weak. **One image. §9a is NOT rewritten on it.** **(iii)** I1/I2 independence — `measure_pass7.py:352-353` sets them to the same boolean, so the gauge grades its own success.
