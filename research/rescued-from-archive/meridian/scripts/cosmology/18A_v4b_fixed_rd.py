#!/usr/bin/env python3
"""
18A v4b: Fixed-r_d Anchoring Test (Lee 2025 reproduction + extension)
=====================================================================
Identical to v4 but adds a Gaussian prior on the sound horizon:
  r_d = 147.09 +/- 0.26 Mpc (Planck DR3, arXiv:2507.01380)

Motivation: Lee (2025, arXiv:2507.01380, Annals Phys. 489) showed that
the DESI DR2 preference for dynamical dark energy vanishes under fixed-r_d
anchoring. Lee used BAO-only. We extend this to the full multi-probe
dataset (BAO + Pantheon+ + CMB + fsigma8), making it a stronger test.

If DAIC collapses (v4b << v4) -> signal is sound horizon tension, not DE.
If DAIC persists (v4b ~ v4) -> signal is genuine w(z) evolution.

Fit A (Meridian): constant w0, GR perturbations (mu=Sigma=1). 3 params.
Fit B (CPL):      w0 + wa, perturbations coupled to w(z).    4 params.
Both fits receive the same r_d prior -> tests whether r_d freedom drives DAIC.

Data:
  - DESI DR2 BAO (official Cobaya: 13 data points, full covariance)
  ...
  - Planck DR3 r_d prior: 147.09 +/- 0.26 Mpc (NEW)

Authors: Clayton W. Iggulden-Schnell & Clawd
Date: 2026-03-21 (Phase 19B.7)
"""
...
def get_desi_dr2_bao():
    """DESI DR2 BAO: official Cobaya likelihood data (GCcomb).

    Source: github.com/CobayaSampler/bao_data/tree/master/desi_bao_dr2
    Full-precision mean values + 13x13 covariance matrix.

    Data vector order (13 elements):
      0: BGS  z=0.295  DV/rd  (isotropic)
      1: LRG1 z=0.510  DM/rd
      2: LRG1 z=0.510  DH/rd
      3: LRG2 z=0.706  DM/rd
      4: LRG2 z=0.706  DH/rd
      5: LRG3+ELG1 z=0.934 DM/rd
      6: LRG3+ELG1 z=0.934 DH/rd
      7: ELG2 z=1.321  DM/rd
      8: ELG2 z=1.321  DH/rd
      9: QSO  z=1.484  DM/rd
     10: QSO  z=1.484  DH/rd
     11: Lya  z=2.330  DH/rd  (NOTE: DH before DM in Cobaya file)
     12: Lya  z=2.330  DM/rd
    """
    data_dir = os.path.join(OUTPUT_DIR, 'data', 'desi_dr2_bao')
    mean_file = os.path.join(data_dir, 'desi_gaussian_bao_ALL_GCcomb_mean.txt')
    cov_file = os.path.join(data_dir, 'desi_gaussian_bao_ALL_GCcomb_cov.txt')

    print("  Loading DESI DR2 BAO (official Cobaya data)...", flush=True)

    # Parse mean values: columns are z, value, quantity_name
    values = []
    ...
    mean_vec = np.array(values)
    print(f"    {len(mean_vec)} BAO measurements loaded", flush=True)

    # Load full covariance and invert
    cov = np.loadtxt(cov_file)
    cov_inv = np.linalg.inv(cov)
    print(f"    {cov.shape[0]}x{cov.shape[1]} covariance (cond={np.linalg.cond(cov):.1e})",
          flush=True)

    # Redshifts for theory predictions
    iso_z = np.array([0.295])
    aniso_z = np.array([0.510, 0.706, 0.934, 1.321, 1.484, 2.330])

    return {
        'mean_vec': mean_vec,     # 13-element data vector
        'cov_inv': cov_inv,       # 13x13 inverse covariance
        'iso_z': iso_z,           # redshifts needing DV prediction
        'aniso_z': aniso_z,       # redshifts needing DM/DH prediction
        'n_points': len(mean_vec),
    }
...
def get_camb_predictions(w0, wa, Om, H0, sne_z_cmb, sne_z_hel,
    ...
    try:
        ...
        zs_for_power = [0.0, 0.067, 0.295, 0.38, 0.51, 0.61, 0.70, 0.706,
                        0.85, 0.934, 1.321, 1.48, 1.484, 2.33]
        ...
        # BAO: build 13-element theory vector matching Cobaya data order
        # [BGS_DV, LRG1_DM, LRG1_DH, LRG2_DM, LRG2_DH, LRG3E1_DM, LRG3E1_DH,
        #  ELG2_DM, ELG2_DH, QSO_DM, QSO_DH, Lya_DH, Lya_DM]
        rd = results.get_derived_params()['rdrag']
        c_km = 299792.458

        def DM_rd(z):
            return results.comoving_radial_distance(z) / rd

        def DH_rd(z):
            return c_km / results.hubble_parameter(z) / rd

        def DV_rd(z):
            DM = results.comoving_radial_distance(z)
            DH = c_km / results.hubble_parameter(z)
            return (z * DM**2 * DH)**(1.0/3.0) / rd

        pred['bao_vec'] = np.array([
            DV_rd(0.295),                               # 0: BGS DV
            DM_rd(0.510), DH_rd(0.510),                 # 1-2: LRG1
            DM_rd(0.706), DH_rd(0.706),                 # 3-4: LRG2
            DM_rd(0.934), DH_rd(0.934),                 # 5-6: LRG3+ELG1
            DM_rd(1.321), DH_rd(1.321),                 # 7-8: ELG2
            DM_rd(1.484), DH_rd(1.484),                 # 9-10: QSO
            DH_rd(2.330), DM_rd(2.330),                 # 11-12: Lya (DH first!)
        ])
        pred['rd'] = rd  # sound horizon in Mpc (for r_d prior)
...
def log_like_bao(pred, data):
    """BAO chi-squared using full 13x13 Cobaya covariance matrix."""
    delta = pred['bao_vec'] - data['mean_vec']
    chi2 = delta @ data['cov_inv'] @ delta
    return -0.5 * chi2
...
# Planck DR3 sound horizon prior (Lee 2025, arXiv:2507.01380)
RD_PRIOR_MEAN = 147.09  # Mpc
RD_PRIOR_SIGMA = 0.26   # Mpc

def log_prior_rd(pred):
    """Gaussian prior on r_d from Planck DR3."""
    return -0.5 * ((pred['rd'] - RD_PRIOR_MEAN) / RD_PRIOR_SIGMA)**2
...
def run_mcmc(fit_type, bao_data, sne_data, cmb_data, fs8_data,
    ...
    chain_path = os.path.join(OUTPUT_DIR, f'18A_v4b_chain_{fit_type}.npz')
...
def analyze_results(resultA, resultB, bao_data, sne_data, cmb_data, fs8_data):
    ...
    print(f"\n{'='*70}")
    print("18A v4b MCMC -- FIXED r_d TEST (DR2 BAO + Pantheon+ + r_d prior)")
    ...
    # Data points: 13 BAO (official Cobaya vector) + (1590-1) SNe + 3 CMB + 7 fs8
    # The -1 is for M marginalization (one degree of freedom removed)
    n_bao = bao_data['n_points']  # 13
...
def make_plots(resultA, resultB, comparison, sne_data):
    ...
    fig.suptitle('18A v4: DR2 BAO + Full Pantheon+ Covariance',
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, '18A_v4b_results.png')
...
def main():
    print("=" * 70)
    print("18A v4b: FIXED r_d ANCHORING TEST (DR2 BAO + r_d prior)")
    ...
    n_bao = bao_data['n_points']
    n_sne_eff = sne_data['N'] - 1  # M marginalized
    n_cmb = 3
    n_fs8 = len(fs8_data['z'])
    print(f"\n  BAO: {n_bao} points (official Cobaya DR2 GCcomb, full 13x13 covariance)")
    ...
    print(f"  r_d prior: {RD_PRIOR_MEAN} +/- {RD_PRIOR_SIGMA} Mpc (Planck DR3)")
    print(f"  Total: {n_bao + n_sne_eff + n_cmb + n_fs8} effective data points + r_d prior",
          flush=True)
    ...
    # Save results
    rpath = os.path.join(OUTPUT_DIR, '18A_v4b_results.md')
    with open(rpath, 'w', encoding='utf-8') as f:
        f.write("# 18A v4b Results: Fixed r_d Anchoring Test\n\n")
        f.write(f"**Date:** {time.strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**Runtime:** {(time.time()-t_start)/3600:.1f} hours\n")
        f.write(f"**BAO:** DESI DR2 -- official Cobaya GCcomb + r_d prior (147.09+/-0.26)\n"
                f"**SNe:** Full Pantheon+ (1590 SNe, stat+sys covariance, M marginalized)\n\n")
    ...
    print(f"\n{'='*70}")
    print(f"18A v4b COMPLETE. Runtime: {t_total/3600:.1f} hours ({t_total:.0f}s)")
    print(f"{'='*70}", flush=True)
