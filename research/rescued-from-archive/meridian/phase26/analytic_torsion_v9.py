...
def process_bundle(n_twist, max_deg, use_balanced=True, label_suffix=""):
    """
...
print(f"\n  === FS METRIC (baseline, same as v8) ===")
eigs_FS = process_bundle(0, MAX_DEGREE, use_balanced=False, label_suffix=" [FS]")

print(f"\n  === BALANCED METRIC (k={BAL_K}) ===")
eigs_bal = process_bundle(0, MAX_DEGREE, use_balanced=True, label_suffix=" [bal]")

# Also do O(5) and O(-5) with balanced metric
print(f"\n  === O(5) with balanced metric ===")
eigs_5_bal = process_bundle(5, MAX_DEGREE, use_balanced=True, label_suffix=" [bal]")

print(f"\n  === O(-5) with balanced metric ===")
eigs_m5_bal = process_bundle(-5, MAX_DEGREE, use_balanced=True, label_suffix=" [bal]")
...
outfile = 'analytic_torsion_v9_results.json'
...
if eigs_FS is not None and eigs_bal is not None:
    ...
    pos_bal = eigs_bal[eigs_bal > 0.5]
    if len(pos_FS) > 0 and len(pos_bal) > 0:
        print(f"  KEY RESULT: lambda_1(FS)={pos_FS[0]:.4f} -> lambda_1(bal)={pos_bal[0]:.4f} "
              f"(KE={1.5:.4f})")
print(f"{'='*72}")
