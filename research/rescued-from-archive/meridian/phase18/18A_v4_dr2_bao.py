#!/usr/bin/env python3
"""
18A v4: Decoupled Perturbation Test with DESI DR2 BAO + Full Pantheon+
======================================================================
Upgrade from v3: replaces DESI DR1 BAO with actual DR2 values.
  - Official Cobaya likelihood data (GCcomb) from CobayaSampler/bao_data
  - Full 13x13 covariance matrix (not just per-bin correlations)
  - All 7 DR2 tracers: BGS (DV), LRG1/LRG2/LRG3+ELG1/ELG2/QSO/Lya (DM+DH)
  - QSO now anisotropic (was isotropic-only in DR1)
  - Pantheon+, CMB, and fσ8 unchanged from v3
...
Date: 2026-03-21 (upgrade from 2026-03-20 v3)
"""
...
def run_mcmc(fit_type, bao_data, sne_data, cmb_data, fs8_data,
    ...
    chain_path = os.path.join(OUTPUT_DIR, f'18A_v4_chain_{fit_type}.npz')
...
def analyze_results(resultA, resultB, bao_data, sne_data, cmb_data, fs8_data):
    ...
    print(f"\n{'='*70}")
    print("18A v4 MCMC -- FINAL RESULTS (DR2 BAO + Full Pantheon+)")
...
def make_plots(resultA, resultB, comparison, sne_data):
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, '18A_v4_results.png')
...
def main():
    print("=" * 70)
    print("18A v4: DECOUPLED PERTURBATION TEST (DR2 BAO)")
    ...
    # Save results
    rpath = os.path.join(OUTPUT_DIR, '18A_v4_results.md')
    with open(rpath, 'w', encoding='utf-8') as f:
        f.write("# 18A v4 Results: DR2 BAO + Full Pantheon+ Covariance\n\n")
        f.write(f"**Date:** {time.strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**Runtime:** {(time.time()-t_start)/3600:.1f} hours\n")
        f.write(f"**BAO:** DESI DR2 -- official Cobaya GCcomb likelihood (13x13 covariance)\n"
    ...
    print(f"\n{'='*70}")
    print(f"18A v4 COMPLETE. Runtime: {t_total/3600:.1f} hours ({t_total:.0f}s)")
    print(f"{'='*70}", flush=True)
