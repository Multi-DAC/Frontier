...
# This file was *autogenerated* from the file /mnt/c/Users/mercu/clawd/projects/Project Meridian/phase21/box1_spectrum.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_6 = Integer(6); _sage_const_5 = Integer(5); _sage_const_15 = Integer(15); _sage_const_20 = Integer(20); _sage_const_25 = Integer(25)
# ===================================================================
...
V10 = W(_sage_const_1 ,_sage_const_0 )  # fundamental 3
V01 = W(_sage_const_0 ,_sage_const_1 )  # anti-fundamental 3-bar
V11 = W(_sage_const_1 ,_sage_const_1 )  # adjoint 8
...
# C_2(a,b) = (a^2 + ab + b^2 + 3a + 3b) / 3
def casimir(a, b):
    return (a**_sage_const_2  + a*b + b**_sage_const_2  + _sage_const_3 *a + _sage_const_3 *b) / _sage_const_3 

print(f"\nCasimir checks:")
print(f"C_2(1,0) = {float(casimir(_sage_const_1 ,_sage_const_0 ))}")  # should be 4/3
print(f"C_2(0,1) = {float(casimir(_sage_const_0 ,_sage_const_1 ))}")  # should be 4/3
print(f"C_2(1,1) = {float(casimir(_sage_const_1 ,_sage_const_1 ))}")  # should be 3
print(f"C_2(2,2) = {float(casimir(_sage_const_2 ,_sage_const_2 ))}")  # should be 8 = 2*4
...
print(f"{'l':>3} {'(a,b)':>8} {'C_2':>8} {'l(l+_sage_const_2 )':>8} {'dim':>8} {'(l+_sage_const_1 )**_sage_const_3 ':>8}")
for l in range(_sage_const_6 ):
    a, b = l, l
    c2 = casimir(a, b)
    dim_ab = W(a,b).degree()
    print(f"{l:3d} ({a},{b}){' '*(4-len(str(a))-len(str(b)))} {float(c2):8.1f} {l*(l+_sage_const_2 ):8d} {dim_ab:8d} {(l+_sage_const_1 )**_sage_const_3 :8d}")
...
def weights_of_irrep(a, b):
    ...
    for wt, mult in wt_dict.items():
        ...
        w1 = QQ(_sage_const_2 )/_sage_const_3  * coeffs[_sage_const_0 ] + QQ(_sage_const_1 )/_sage_const_3  * coeffs[_sage_const_1 ]
        w2 = -QQ(_sage_const_1 )/_sage_const_3  * coeffs[_sage_const_0 ] + QQ(_sage_const_1 )/_sage_const_3  * coeffs[_sage_const_1 ]
        w3 = -QQ(_sage_const_1 )/_sage_const_3  * coeffs[_sage_const_0 ] - QQ(_sage_const_2 )/_sage_const_3  * coeffs[_sage_const_1 ]
...
def branching_multiplicity_spin_half(a, b, q_target):
    ...
    for (w1, w2, w3), mult in weights:
        if w3 == -q_target:
            ...
            su2_weights[m_su2] = su2_weights.get(m_su2, _sage_const_0 ) + mult
    ...
    while True:
        ...
        for mm in range(int(j), int(-j)-_sage_const_1 , -_sage_const_1 ):
            mm_rat = QQ(mm)
            if mm_rat not in su2_wts or su2_wts[mm_rat] <= _sage_const_0 :
                # Inconsistency - this shouldn't happen for a valid decomposition
                print(f"WARNING: inconsistency at (a,b)=({a},{b}), j={j}, m={mm}")
                break
            su2_wts[mm_rat] -= _sage_const_1 
            if su2_wts[mm_rat] == _sage_const_0 :
                del su2_wts[mm_rat]
...
for a in range(_sage_const_5 ):
    for b in range(_sage_const_5 ):
        # For scalars on O(0): need (j=0, q=0)
        weights = weights_of_irrep(a, b)
        # Count singlets with q=0
        su2_weights_q0 = {}
        for (w1, w2, w3), mult in weights:
            if w3 == _sage_const_0 :  # q = -w3 = 0
                m_su2 = w1 - w2
                su2_weights_q0[m_su2] = su2_weights_q0.get(m_su2, _sage_const_0 ) + mult

        # Count spin-0 (singlet) components
        if su2_weights_q0:
            # Peel off irreps
            wts = dict(su2_weights_q0)
            singlet_count = _sage_const_0 
            while True:
                max_m = None
                for m in sorted(wts.keys(), reverse=True):
                    if wts[m] > _sage_const_0 :
                        max_m = m
                        break
                if max_m is None:
                    break
                ...
                for mm in range(int(j), int(-j)-_sage_const_1 , -_sage_const_1 ):
                    mm_r = QQ(mm)
                    if mm_r in wts and wts[mm_r] > _sage_const_0 :
                        wts[mm_r] -= _sage_const_1 
                if j == _sage_const_0 :
                    singlet_count += _sage_const_1 

            if singlet_count > _sage_const_0 :
...
for a in range(_sage_const_15 ):
    for b in range(_sage_const_15 ):
        if a + b > _sage_const_20 :
            continue
        mult = branching_multiplicity_spin_half(a, b, -_sage_const_1 )
...
for c2, total, a, b, br in eigenvalues_O0[:_sage_const_25 ]:
...
eigenvalues_O2.sort()
print(f"{'C_2':>8} {'total_mult':>10} {'(a,b)':>8} {'dim':>6} {'branch':>6}")
for c2, total, a, b, br in eigenvalues_O2[:_sage_const_25 ]:
    print(f"{c2:8.3f} {total:10d} ({a},{b}){' '*(4-len(str(a))-len(str(b)))} {W(a,b).degree():6d} {br:6d}")
...
for c2 in sorted(ev_groups_O0.keys())[:_sage_const_20 ]:
...
for c2 in sorted(ev_groups_O2.keys())[:_sage_const_20 ]:
