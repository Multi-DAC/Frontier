...
# This file was *autogenerated* from the file torsion_setup.sage
from sage.all_cmdline import *   # import sage library

_sage_const_100 = Integer(100); _sage_const_70 = Integer(70); _sage_const_6 = Integer(6); _sage_const_1 = Integer(1); _sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_9 = Integer(9); _sage_const_12 = Integer(12); _sage_const_4 = Integer(4); _sage_const_20 = Integer(20)
...
n = _sage_const_6   # rank

# Intersection matrix
Q = diagonal_matrix(ZZ, [_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 ])
print(f"Intersection form Q = diag(1, -1, -1, -1, -1, -1)")
print(f"Signature: (1, 5)")

# Anticanonical class -K_S
neg_K = vector(ZZ, [_sage_const_3 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 ])
print(f"-K_S = {neg_K}  (= 3H - E1 - E2 - E3 - E4 - E5)")
print(f"(-K)^2 = {neg_K * Q * neg_K}  (= 9 - 5 = 4)")

# Hypercharge flux
c1_LY = vector(ZZ, [_sage_const_2 , -_sage_const_1 , -_sage_const_1 , -_sage_const_1 , -_sage_const_2 , _sage_const_0 ])
print(f"\nc1(L_Y) = {c1_LY}")
...
n_blowups = _sage_const_5 
c1_sq = _sage_const_9  - n_blowups
c2_S = _sage_const_3  + n_blowups  # = chi_top

print(f"dP_{n_blowups}:")
print(f"  c1(S)^2 = (-K)^2 = {c1_sq}")
print(f"  c2(S) = chi_top = {c2_S}")
print(f"  chi(O_S) = (c1^2 + c2)/12 = ({c1_sq} + {c2_S})/12 = {(c1_sq + c2_S)/_sage_const_12 }")
print(f"  h^{_sage_const_0 ,_sage_const_0 } = 1, h^{_sage_const_1 ,_sage_const_0 } = 0, h^{_sage_const_2 ,_sage_const_0 } = 0")
print(f"  h^{_sage_const_1 ,_sage_const_1 } = {c2_S - _sage_const_2 } = {c2_S - _sage_const_2 }")
print(f"  Picard number rho = {_sage_const_1  + n_blowups}")

# Chern numbers
print(f"\n  Chern numbers:")
print(f"    c1^2 = {c1_sq}")
print(f"    c2 = {c2_S}")
print(f"    Signature tau = (c1^2 - 2*c2)/3 = ({c1_sq} - {_sage_const_2 *c2_S})/3 = {(c1_sq - _sage_const_2 *c2_S)/_sage_const_3 }")
...
def chi_HRR(c1_L):
    ...
    return _sage_const_1  + c1L_dot_negK / _sage_const_2  + c1L_sq / _sage_const_2 

# Trivial bundle O_S
c1_O = vector(ZZ, [_sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 ])
chi_O = chi_HRR(c1_O)
...
# Powers of L_Y
print(f"\nPowers of L_Y:")
for k in range(-_sage_const_3 , _sage_const_4 ):
    c1_k = k * c1_LY
    chi_k = chi_HRR(c1_k)
    c1k_sq = c1_k * Q * c1_k
...
print(f"L_Y effectivity: c1.H = {c1_LY[_sage_const_0 ]} > 0, c1.(-K) = {c1_LY * Q * neg_K} > 0")
...
T3_adj = _sage_const_3 
T2_adj = _sage_const_2 
T3_bf = _sage_const_2 
T2_bf = _sage_const_3 
...
N_Y = -_sage_const_3 
delta_3 = _sage_const_0 
delta_2 = QQ(N_Y) / QQ(_sage_const_2 )
delta_1 = QQ(_sage_const_3 ) / QQ(_sage_const_5 ) * QQ(N_Y) / QQ(_sage_const_2 )
...
var('s', 'c')
target = RR(log(_sage_const_3 ) / sqrt(_sage_const_2 ))
...
ch2_LY = QQ(c1_LY * Q * c1_LY) / QQ(_sage_const_2 )  # = -3/2
...
genus1_curves = []
for d in range(_sage_const_1 , _sage_const_5 ):
    rng = range(-d-_sage_const_1 , d+_sage_const_2 )
    for ms in iterproduct(rng, repeat=_sage_const_5 ):
        beta = vector(ZZ, [d] + list(ms))
        beta_sq = beta * Q * beta
        beta_negK = beta * Q * neg_K
        if beta_sq == beta_negK and beta_negK > _sage_const_0 :
...
for beta in sorted(genus1_curves, key=lambda b: b*Q*neg_K)[:_sage_const_20 ]:
    beta_sq = beta * Q * beta
    beta_negK = beta * Q * neg_K
