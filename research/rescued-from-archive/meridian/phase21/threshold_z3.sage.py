...
# This file was *autogenerated* from the file threshold_z3.sage
from sage.all_cmdline import *   # import sage library

_sage_const_200 = Integer(200); _sage_const_60 = Integer(60); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_4 = Integer(4); _sage_const_24 = Integer(24); _sage_const_100 = Integer(100); _sage_const_0 = Integer(0); _sage_const_1p5 = RealNumber('1.5'); _sage_const_1p2 = RealNumber('1.2'); _sage_const_1p1 = RealNumber('1.1'); _sage_const_1p05 = RealNumber('1.05'); _sage_const_1p01 = RealNumber('1.01'); _sage_const_5 = Integer(5); _sage_const_6 = Integer(6); _sage_const_1en30 = RealNumber('1e-30'); _sage_const_0p01 = RealNumber('0.01')
...
print(f"|eta(omega)|^2 = {eta_omega_abs**_sage_const_2 }")
print(f"|eta(omega)|^4 = {eta_omega_abs**_sage_const_4 }")

# Verify numerically
q_omega = C(exp(_sage_const_2  * pi * I * omega))
print(f"\nq = e^{_sage_const_2 *pi*i*omega} : |q| = {abs(q_omega)}")

# Direct numerical computation of eta(omega) for verification
# eta(tau) = q^{1/24} * prod_{n=1}^{inf} (1 - q^n), q = e^{2*pi*i*tau}
q = q_omega
eta_numerical = q**(C(_sage_const_1 )/C(_sage_const_24 ))
for n in range(_sage_const_1 , _sage_const_200 ):
    eta_numerical *= (_sage_const_1  - q**n)
...
# ============================================================
T2 = R(sqrt(_sage_const_3 )) / R(_sage_const_2 )
...
tau_third = omega / _sage_const_3 
q_third = C(exp(_sage_const_2  * pi * I * tau_third))
print(f"\nomega/3 = {tau_third}")
print(f"|q(omega/3)| = {abs(q_third)}")

eta_third = q_third**(C(_sage_const_1 )/C(_sage_const_24 ))
for n in range(_sage_const_1 , _sage_const_200 ):
    eta_third *= (_sage_const_1  - q_third**n)
print(f"|eta(omega/3)| = {abs(eta_third)}")

# ============================================================
# The moduli-dependent volume factor
# ============================================================
V = (T2 / R(_sage_const_3 )) * abs(eta_third)**_sage_const_4  * T2 * eta_omega_abs**_sage_const_4 
...
T2_val = R(sqrt(_sage_const_3 ))/R(_sage_const_2 )
eta4 = eta_omega_abs**_sage_const_4 
gamma_E = R(euler_gamma)

zeta_prime_0_unshifted = -ln(T2_val * eta4) - ln(R(_sage_const_4 )*pi_val/exp(gamma_E))
...
shift_a = R(_sage_const_1 ) / R(_sage_const_3 )  # Wilson line shift

# Compute shifted lattice sum truncated at large N
N_max = _sage_const_100 
shifted_sum = R(_sage_const_0 )
unshifted_sum = R(_sage_const_0 )

for m in range(-N_max, N_max + _sage_const_1 ):
    for n in range(-N_max, N_max + _sage_const_1 ):
        if m == _sage_const_0  and n == _sage_const_0 :
            # Unshifted: skip (0,0)
            pass
        else:
            # Unshifted lattice point
            z_unsft = C(m) + C(n) * omega
            unshifted_sum += R(_sage_const_1 ) / abs(z_unsft)**_sage_const_2 

        # Shifted lattice point (always include (0,0) term)
        z_sft = C(m) + C(n) * omega + C(shift_a)
        shifted_sum += R(_sage_const_1 ) / abs(z_sft)**_sage_const_2 
...
s_values = [R(_sage_const_1p5 ), R(_sage_const_1p2 ), R(_sage_const_1p1 ), R(_sage_const_1p05 ), R(_sage_const_1p01 )]

for s in s_values:
    shifted = R(_sage_const_0 )
    unshifted = R(_sage_const_0 )
    for m in range(-N_max, N_max + _sage_const_1 ):
        for n in range(-N_max, N_max + _sage_const_1 ):
            if not (m == _sage_const_0  and n == _sage_const_0 ):
                z_unsft = C(m) + C(n) * omega
                unshifted += R(_sage_const_1 ) / abs(z_unsft)**(_sage_const_2 *s)
            z_sft = C(m) + C(n) * omega + C(shift_a)
            shifted += R(_sage_const_1 ) / abs(z_sft)**(_sage_const_2 *s)
...
z_val = R(_sage_const_1 ) / R(_sage_const_3 )  # a = 1/3
q_val = C(exp(_sage_const_2  * pi * I * omega))

theta1 = C(_sage_const_0 )
for n in range(_sage_const_200 ):
    sign = (-_sage_const_1 )**n
    q_power = (n + R(_sage_const_1 )/R(_sage_const_2 ))**_sage_const_2 
    sine_arg = (_sage_const_2 *n + _sage_const_1 ) * pi_val * z_val
    theta1 += C(sign) * q_val**(q_power/R(_sage_const_2 )) * C(sin(sine_arg))
theta1 *= C(_sage_const_2 )

print(f"\ntheta_1(1/3 | omega) = {theta1}")
print(f"|theta_1(1/3 | omega)| = {abs(theta1)}")
print(f"|theta_1(1/3 | omega)|^2 = {abs(theta1)**_sage_const_2 }")

# The ratio |theta_1/eta|^2:
ratio_theta_eta = abs(theta1)**_sage_const_2  / eta_omega_abs**_sage_const_2 
...
phi = R(_sage_const_1 ) / R(_sage_const_3 )
q_Y = R(_sage_const_5 ) / R(_sage_const_6 )
...
def compute_theta1(z, tau, N_terms=_sage_const_200 ):
    ...
    for n in range(N_terms):
        ...
        s_arg = (_sage_const_2 *n + _sage_const_1 ) * R(pi) * R(z)
...
print(f"Check: |theta_+| = |theta_-|? {abs(abs(theta_plus) - abs(theta_minus)) < _sage_const_1en30 }")
...
b3_adj = R(_sage_const_8 )   # dim(adj SU(3)) = 8, but actually it's the Dynkin index C_2(adj)
b2_adj = R(_sage_const_3 )   # dim(adj SU(2)) = 3, actually C_2(adj(SU(2))) = 2
...
T3_adj = R(_sage_const_3 )  # From (8,1) only
T2_adj = R(_sage_const_2 )  # From (1,3) only

# Bifundamental contribution (Y=5/6):
T3_bifund = R(_sage_const_1 )     # T_3(fund) * dim(fund_2) = 1/2 * 2
T2_bifund = R(_sage_const_3 )/R(_sage_const_2 )  # T_2(fund) * dim(fund_3) = 1/2 * 3

# Including conjugate (same Dynkin indices):
T3_bifund_total = _sage_const_2  * T3_bifund  # = 2
T2_bifund_total = _sage_const_2  * T2_bifund  # = 3
...
ln_adj_factor = ln(T2_val * eta_omega_abs**_sage_const_4 )
ln_bifund_factor = ln(abs(theta_plus)**_sage_const_2  / eta_omega_abs**_sage_const_2 )
...
# ============================================================
target = ln(R(_sage_const_3 )) / sqrt(R(_sage_const_2 ))
...
print(f"Ratio (diff / ln(3)) = {float(diff / ln(R(_sage_const_3 ))):.15f}")
print(f"Ratio (diff / sqrt(2)) = {float(diff / sqrt(R(_sage_const_2 ))):.15f}")
...
print(f"\nDirect match: {'YES' if abs(float(geometric_ratio) - float(target)) / float(target) < _sage_const_0p01  else 'NO'}")
...
for name, val in [
    ...
    ("(Delta_3 - Delta_2) / (4*pi^2)", diff / (_sage_const_4 *pi_val**_sage_const_2 )),
    ("(Delta_3 - Delta_2) / (8*pi^2)", diff / (_sage_const_8 *pi_val**_sage_const_2 )),
    ...
    ("2*ln(|eta(omega)|)", _sage_const_2 *ln(eta_omega_abs)),
    ("-ln(T_2 |eta|^4) / ln(|theta/eta|^2)", -ln_adj_factor / ln_bifund_factor if ln_bifund_factor != _sage_const_0  else R(_sage_const_0 )),
]:
    v = float(val)
    if abs(float(target)) > _sage_const_0 :
        pct = abs(v - float(target)) / float(target) * _sage_const_100 
...
print(f"|theta_1/eta|^2 = {float(abs(theta_plus)**_sage_const_2 /eta_omega_abs**_sage_const_2 ):.15f}")
...
print(f"Gamma(1/3)^{_sage_const_3 /_sage_const_2 } = {float(gamma_13**(R(_sage_const_3 )/R(_sage_const_2 ))):.15f}")
print(f"3^{_sage_const_1 /_sage_const_8 } = {float(R(_sage_const_3 )**(R(_sage_const_1 )/R(_sage_const_8 ))):.15f}")
print(f"|theta_1(5/18|omega)| / (3^{_sage_const_1 /_sage_const_8 } * Gamma(1/3)^{_sage_const_3 /_sage_const_2 } / (2*pi)) = {float(abs(theta_plus) / eta_omega_abs):.15f}")
