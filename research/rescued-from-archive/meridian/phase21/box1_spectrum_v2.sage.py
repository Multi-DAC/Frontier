...
# This file was *autogenerated* from the file /mnt/c/Users/mercu/clawd/projects/Project Meridian/phase21/box1_spectrum_v2.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_20 = Integer(20); _sage_const_25 = Integer(25); _sage_const_30 = Integer(30)
...
def get_weights_epsilon(a, b):
    ...
    for wt, mult in wt_dict.items():
        ...
        w1 = QQ(_sage_const_2 *m1 + m2) / _sage_const_3 
        w2 = QQ(-m1 + m2) / _sage_const_3 
        w3 = QQ(-m1 - _sage_const_2 *m2) / _sage_const_3 
...
def branching_mult_j1(a, b, q_target):
    ...
    while True:
        ...
        for mm in range(j, -j-_sage_const_1 , -_sage_const_1 ):
            wts[QQ(mm)] = wts.get(QQ(mm), _sage_const_0 ) - _sage_const_1 
...
for l in range(_sage_const_8 ):
    ...
    while True:
        ...
        for mm in range(j, -j-_sage_const_1 , -_sage_const_1 ):
            wts[QQ(mm)] = wts.get(QQ(mm), _sage_const_0 ) - _sage_const_1 
        if j == _sage_const_0 :
            singlets += _sage_const_1 

    if singlets > _sage_const_0 :
        dim_v = W(a,b).degree()
...
for a in range(_sage_const_20 ):
    for b in range(_sage_const_20 ):
        if a + b > _sage_const_25 :
            continue
        mult = branching_mult_j1(a, b, -_sage_const_1 )
...
for c2, total, a, b, br, dim_v in results_O0[:_sage_const_30 ]:
...
print(f"{'C_2':>10} {'total_mult':>10} {'(a,b)':>8} {'dim_V':>7} {'br_mult':>7}")
for c2, total, a, b, br, dim_v in results_O2[:_sage_const_30 ]:
    print(f"{c2:10.4f} {total:10d} ({a},{b}){' '*(4-len(str(a))-len(str(b)))} {dim_v:7d} {br:7d}")
...
for lam in sorted(ev_O0.keys())[:_sage_const_20 ]:
...
for lam in sorted(ev_O2.keys())[:_sage_const_20 ]:
...
for lam, mult in sorted_O0[:_sage_const_30 ]:
    print(f"  {lam:.6f}, {mult}")

print("\nFor O(2) (0,1)-forms:")
print("lambda_n, d_n (first 30 eigenvalue-multiplicity pairs):")
sorted_O2 = sorted(ev_O2.items())
for lam, mult in sorted_O2[:_sage_const_30 ]:
    print(f"  {lam:.6f}, {mult}")
